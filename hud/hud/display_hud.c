...
/*   Updated: 2020/02/20 16:26:29 by calin            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	print_ammo(t_data *data)
{
	...
	tmpstr = ft_strjoin(ft_itoa(data->current_ammo), " | ", 1);
	str = ft_strjoin(tmpstr, ft_itoa(data->total_ammo), 3);
...
int		print_menu(t_data *data)
{
	...
	while (++i < 4)
		...
		data->menu_texture[i].img.addrs, data->menu_texture[i].pos_text.x,
...
void	print_inventory(t_data *data)
{
	...
	if (data->inventory.slot_one != 0)
	{
		...
		0xFFFFFF, (tmp = ft_strjoin("x", str, 2)));
		mlx_put_image_to_window(data->mlx_addrs, data->win_addrs,
		data->tab_tex[9].img.addrs, data->pos_texture[9].x,
	...
	if (data->inventory.slot_two != 0)
	{
		...
		0xFFFFFF, (tmp = ft_strjoin("x", str, 2)));
		mlx_put_image_to_window(data->mlx_addrs, data->win_addrs,
		data->tab_tex[10].img.addrs, data->pos_texture[10].x,
...
int		print_hud(t_data *data)
{
	...
	while (++i < text_to_print)
		mlx_put_image_to_window(data->mlx_addrs, data->win_addrs,
		data->tab_tex[i].img.addrs, data->pos_texture[i].x,
